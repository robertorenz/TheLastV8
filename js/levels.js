...
(function (root) {
  ...
  const LEVELS = [
    {
      // Traced from the original surface map (800x380 px -> 160x64 tiles). Every road that runs off the right edge
      // comes back in on the left at the same height, so the valley is one continuous loop: two dashed highways along
      // the top, an upper and a lower winding road, the river, and a track along the bottom. Geometry is in tiles
  ...
  function smoothPath(pts, loop, period, unit = TILE) {
    const n = pts.length;
    if (n < 2) return pts.map((p) => [p[0] * unit, p[1] * unit]);
    const ext = loop && period
      ? [[pts[n - 2][0] - period, pts[n - 2][1]], ...pts, [pts[1][0] + period, pts[1][1]]]
      : [pts[0], ...pts, pts[n - 1]];
    return sampleCatmull(ext, false, unit);
  }
  function smoothLoop(pts, unit = TILE) {
    const n = pts.length;
    return sampleCatmull([pts[n - 1], ...pts, pts[0], pts[1]], true, unit);
  }
  function sampleCatmull(ext, closed, unit = TILE) {
    const out = [], steps = 8;
    ...
    if (!closed) { const last = ext[ext.length - 2]; out.push([last[0] * unit, last[1] * unit]); }
    return out;
  }
